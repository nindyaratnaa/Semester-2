/* Pengembangan dari package Peroustakaan dengan menambahkan: 
    3. Method hitungRoyalti dengan parameter harga 1 buku, yang mengembalikan 
       nilai 10% dari penjualan bulan itu. (1 parameter)
    4. Method hitungRoyalti dengan parameter harga 1 buku dan nilai persen yang 
       ingin diambil dari penjualan bulan itu. (2 parameter)
*/

package Perpustakaan_Version_3;
...
public class MainPerpustakaan {
    public static void main(String[] args) {
        ...
        //Array Harga Buku
        double[] hargaBukuTeknologi = {70000, 85000, 90000, 50000, 65000};
        double[] hargaBukuFilsafat = {85000, 100000, 135000, 68000, 250000};
        double[] hargaBukuSejarah = {75000, 99000, 210000, 78000, 59000};
        double[] hargaBukuAgama = {99000, 67000, 99500, 67000, 55700};
        double[] hargaBukuPsikologi ={59000, 145000, 72000, 91000, 200500};
        double[] hargaBukuPolitik = {199000, 37500, 65000, 78000, 99000};
        double[] hargaBukuFiksi = {120000, 95000, 11000, 45000, 65000};
        ...
        for (int i = 0; i < BukuTeknologi.length; i++) {
            bukuTeknologi[i] = new Buku(BukuTeknologi[i], new Penulis[]{penulisTeknologi[i]}, "Teknologi", sinopsisBukuTeknologi[i], tahunBukuTeknologi[i], hargaBukuTeknologi[i]);
        }

        Buku[] bukuFilsafat = new Buku[BukuFilsafat.length];
        for (int i = 0; i < BukuFilsafat.length; i++) {
            bukuFilsafat[i] = new Buku(BukuFilsafat[i], new Penulis[]{penulisFilsafat[i]}, "Filsafat", sinopsisBukuFilsafat[i], tahunBukuFilsafat[i], hargaBukuFilsafat[i]);
        }

        Buku[] bukuSejarah = new Buku[BukuSejarah.length];
        for (int i = 0; i < BukuSejarah.length; i++) {
            bukuSejarah[i] = new Buku(BukuSejarah[i], new Penulis[]{penulisSejarah[i]}, "Sejarah", sinopsisBukuSejarah[i], tahunBukuSejarah[i], hargaBukuSejarah[i]);
        }

        Buku[] bukuAgama = new Buku[BukuAgama.length];
        for (int i = 0; i < BukuAgama.length; i++) {
            bukuAgama[i] = new Buku(BukuAgama[i], new Penulis[]{penulisAgama[i]}, "Agama", sinopsisBukuAgama[i], tahunBukuAgama[i], hargaBukuAgama[i]);
        }

        Buku[] bukuPsikologi = new Buku[BukuPsikologi.length];
        for (int i = 0; i < BukuPsikologi.length; i++) {
            bukuPsikologi[i] = new Buku(BukuPsikologi[i], new Penulis[]{penulisPsikologi[i]}, "Psikologi", sinopsisBukuPsikologi[i], tahunBukuPsikologi[i], hargaBukuPsikologi[i]);
        }

        Buku[] bukuPolitik = new Buku[BukuPolitik.length];
        for (int i = 0; i < BukuPolitik.length; i++) {
            bukuPolitik[i] = new Buku(BukuPolitik[i], new Penulis[]{penulisPolitik[i]}, "Politik", sinopsisBukuPolitik[i], tahunBukuPolitik[i], hargaBukuPolitik[i]);
        }

        Buku[] bukuFiksi = new Buku[BukuFiksi.length];
        for (int i = 0; i < BukuFiksi.length; i++) {
            bukuFiksi[i] = new Buku(BukuFiksi[i], new Penulis[]{penulisFiksi[i]}, "Fiksi", sinopsisBukuFiksi[i], tahunBukuFiksi[i], hargaBukuFiksi[i]);
        }
        ...
        System.out.println("Apa yang ingin dilakukan:(1/2/3)\n1. Menampilkan sinopsis\n2. Membandingkan buku\n3. Menyalin buku\n4. Menghitung royalti");
        ...
        if (pilihan == 1) {
            ...
            System.out.print("\nPersen royalti yang ingin di hitung:\n1. 10%\n2. Lainnya\nMasukkan pilihan: ");
            int pilihRoyalti = input.nextInt(); //Input pilihan untuk persentase royalti
            input.nextLine(); //Menghapus enter yang tidak perlu
            System.out.print("\nMasukkan kategori buku yang ingin disalin: ");
            String inputKategori = input.nextLine().trim(); //Input untuk kategori 
            System.out.print("Masukkan judul buku: ");
            String inputJudul = input.nextLine(); //Input untuk judul

            Kategori kategoriRoyalti = null; //Inisialisasi variabel
            if (inputKategori.equalsIgnoreCase("Teknologi")) { //Mencocokkan dengan input pengguna
                kategoriRoyalti = kategoriTeknologi;
            } else if (inputKategori.equalsIgnoreCase("Filsafat")) {
                kategoriRoyalti = kategoriFilsafat;
            } else if (inputKategori.equalsIgnoreCase("Sejarah")) {
                kategoriRoyalti = kategoriSejarah;
            } else if (inputKategori.equalsIgnoreCase("Agama")) {
                kategoriRoyalti = kategoriAgama;
            } else if (inputKategori.equalsIgnoreCase("Psikologi")) {
                kategoriRoyalti = kategoriPsikologi;
            } else if (inputKategori.equalsIgnoreCase("Politik")) {
                kategoriRoyalti = kategoriPolitik;
            } else if (inputKategori.equalsIgnoreCase("Fiksi")) {
                kategoriRoyalti = kategoriFiksi;
            }

            if (kategoriRoyalti != null) {
                Buku bukuRoyalti = null;
                for (Buku buku : kategoriRoyalti.getListBuku()) {
                    if (buku.getJudul().equalsIgnoreCase(inputJudul)) { //Mencari judul buku yang sesuai dengan input
                        bukuRoyalti = buku;
                        break;
                    }
                }
                ...
                if (bukuRoyalti != null) {
                    double harga = bukuRoyalti.gethargaBuku(); //Memanggil harga buku (dapat berbeda tiap bukunya)
                    double royalti = 0.0;

                    if (pilihRoyalti == 1) {
                        royalti = bukuRoyalti.hitungRoyalti(harga); //Overloading method dengan 1 parameter
                        System.out.printf("\nRoyalti untuk buku %s dengan harga %,.2f adalah: Rp %,.2f\n", inputJudul, harga, royalti);

                    } else {
                        System.out.print("Masukkan persen royalti (tanpa '%'): ");
                        double persenRoyalti = input.nextDouble(); //Input untuk persen royalti
                        royalti = bukuRoyalti.hitungRoyalti(harga, persenRoyalti); //Overloading method dengan 2 parameter
                        System.out.printf("\nRoyalti untuk buku %s dengan persen %.2f dan harga %,.2f adalah Rp %,.2f", inputJudul, persenRoyalti, harga, royalti);
