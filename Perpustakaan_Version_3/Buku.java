...
public class Buku {
    ...
    public Buku(String judul, Penulis[] penulis, String kategori, String sinopsis, int tahun, double hargaBuku) {
        this.judul = judul;
    ...
    private static int perbandinganSinopsis (String sinopsis1, String sinopsis2) { 
        ...
        int kataSama = 0; //Mencari kata yang sama antar sinopsis
        for (String k1 : kata1) {
            for (String k2 : kata2) {
                kataSama++;
                break;
    ...
    public Buku copy() { //Method untuk duplikat buku
        return new Buku(this.judul, this.penulis, this.kategori, this.sinopsis, this.tahun, this.hargaBuku);
    }

    public double hitungRoyalti(double harga) { //Overloading method dengan 1 parameter
        double royalti = harga * 0.10; //Akan mengembalikan nilai royalti yang 10% dari harga buku
        return royalti;
    }

    public double hitungRoyalti(double harga, double persenRoyalti) { //Overloading method dengan 2 parameter
        double royalti = harga * (persenRoyalti / 100); //Akan mengembalikan nilai yang persennya bebas.
        return royalti;
    }
